...
| `server` | 81 | 가입·로그인·로그아웃, 비밀번호 저장과 시도 제한, 게스트와 계정 전환, 구버전 계정 호환, 계정별 설정 저장·범위 검증·키바인드 검증·손상된 값 폴백, 토큰, **한 사람이 랭크 방 두 개에 동시에 잡히지 않는지(느린 폴링·재입장·취소·경기 종료 후)**, 매칭 페어링과 밴드 확장, 방 상태머신, 연결 끊김, 공격 릴레이와 로그 상한, FT3 정산 1회성, 재대결 방지·허용, 방 권한과 정원, 백분위 랭크, 관전 입·퇴장과 좌석 전환, 채팅 전달·도배 제한·이스케이프, 규칙 범위 검증, KO 귀속, **캐시가 통째로 날아간 뒤에도 이어지는 경기·방 목록·유휴 방 유지** |
...
| `browser` | 58 | 실제 Chromium에서 **가입·로그인 실패·로그아웃·게스트→계정 전환** → 40 LINES 완주 → CPU 대전 → 랭킹·설정·키 리바인드 → 방 생성, 규칙 편집기·채팅·타겟팅 4종·이펙트 발생과 소멸·화면 전환 휘도, 한글에 자간이 벌어지지 않는지·[hidden]이 실제로 숨는지·장식 레이어가 클릭을 삼키지 않는지, 소프트 키보드가 올라왔을 때의 가시 영역·입력창 글자 크기·짧은 창에서의 보드 축소, 설정을 바꾸고 로그아웃한 뒤 다른 기기처럼 다시 들어와도 그대로인지, **보드 테두리가 실제로 보이는 대비인지**, 무오류 확인 |
