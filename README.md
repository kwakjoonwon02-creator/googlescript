...
외부 서비스·서버·빌드 도구 없이 링크 하나로 돌아갑니다.

지연이 아쉬우면 **실시간 릴레이**(`relay/`)를 선택적으로 붙일 수 있습니다.
상대 보드가 300~600ms 대신 50ms 남짓으로 갱신됩니다. 붙이지 않아도 게임은
그대로 동작합니다.
...
| **설정** | DAS / ARR / SDF 핸들링, 키 재설정, 고스트·그리드·화면흔들림·이펙트·효과음 |
...
대시보드가 아니라 아케이드 캐비닛을 목표로 했고, 규칙은 세 가지입니다.

**① 기울기(-9°)가 시그니처.** 누를 수 있는 것 — 버튼, 메뉴 행, 탭, 배지,
라운드 핍, 스코어 스트립 — 만 평행사변형입니다. 나머지는 전부 직각이라
모양만으로 "이건 컨트롤"이 읽힙니다. 글자까지 기울면 읽기 나빠지므로,
기울기는 요소가 아니라 배경 의사요소에만 겁니다.

**② 화면마다 강조색은 하나.** 각 화면이 `--accent` 를 정하고 버튼·바·포커스
링·하이라이트가 전부 그 값을 참조합니다. 솔로는 시안, 멀티는 보라, 랭크는
빨강, 랭킹은 금색, 설정은 초록 — 이동하면 방 전체 색이 바뀝니다.

**③ 검정 위에 평면.** 깊이는 대비와 움직임으로 만듭니다. 그림자가 구조를
지탱하지 않고, 배경에 그라디언트 워시가 없습니다.
...
- 메뉴·통계·랭킹 행이 50ms 간격으로 차례로 딜링됩니다
- 버튼 호버는 색이 바뀌는 게 아니라 왼쪽에서 채워집니다
- TR은 이전 값에서 새 값으로 카운트업합니다 (블리츠 점수도)
- 하드 드롭은 떨어진 거리만큼 잔상을 남기고 착지 순간 흰색으로 번쩍입니다
- 랭크 배지에 주기적으로 광택이 지나갑니다
...
Apps Script에는 소켓이 없습니다. 그래서 기본 배포는 폴링으로 동작하고,
원하면 지연에 민감한 부분만 별도 프로세스로 뺄 수 있습니다.

| | Apps Script만 | + 릴레이 |
|---|---|---|
| 상대 보드 갱신 | 300~600ms | **~50ms** |
| 가비지 도착 | 폴링 주기만큼 지연 | 즉시 |
| 매칭 성사 | 최대 2.2초 | 0.6초 |
| 필요한 것 | 없음 | Node 호스트 + `wss://` |

**나뉘는 기준은 "뜨거운가"입니다.**

| 릴레이 (지연 민감) | Apps Script (영구 데이터) |
|---|---|
| 방·매칭·보드 상태·채팅·관전 | 계정·레이팅·랭크·전적·랭킹 |

**코드 사본은 만들지 않았습니다.** 릴레이는 `src/Rooms.gs` 와
`src/Matchmaking.gs` 를 그대로 읽어 VM에 올리고, 저장소만 메모리 구현으로
바꿔 끼웁니다. 구현이 하나라서 두 배포가 어긋날 수 없습니다.

이걸 가능하게 하려고 `Rooms.gs` 에 이음매 두 개만 냈습니다 — 인증
(`ROOMS_OVERRIDES.authenticate`)과 정산(`.settle`). Apps Script에서는
비어 있어 기존 구현이 쓰이고, 릴레이가 자기 것을 꽂습니다.

비밀키 하나로 양방향을 인증합니다. 들어오는 쪽은 Apps Script가 발급한
서명 티켓, 나가는 쪽은 릴레이가 서명해 `doPost` 로 보내는 정산 요청입니다.
비밀키가 없으면 릴레이 경로 전체가 꺼지고 폴링으로 동작합니다.

설치는 `relay/README.md` 를 보세요. 요약하면 편집기에서
`setupRelay("wss://주소")` 를 실행하고, 나온 비밀키로 릴레이를 띄우면 됩니다.

> **주의**: 릴레이를 붙여도 부정행위는 막지 못합니다. 시뮬레이션은 여전히
> 클라이언트에서 돌고 릴레이는 중계와 판정만 합니다. 막으려면 서버가 엔진을
> 직접 돌려야 하고, 그건 별개의 큰 작업입니다.
...
relay/                실시간 릴레이 (선택). src/ 의 방 로직을 그대로 로드
...
| 릴레이 폴링 주기 | `js_game.html` → `RELAY_SYNC_INTERVAL` |
| 릴레이 티켓 수명·비밀키 | `Relay.gs` → `RELAY` |
| 배치 경기 수, Glicko τ, RD 하한 | `Glicko.gs` → `GLICKO` |
| 랭크 백분위·색상 | `Ranks.gs` → `RANKS.LADDER` |
| 색상·기울기·모션 토큰 | `css.html` → `:root` |
| 화면별 강조색 | `css.html` → `#screen-*` |
...
| `server` | 51 | 계정과 토큰, 매칭 페어링과 밴드 확장, 방 상태머신, 연결 끊김, 공격 릴레이와 로그 상한, FT3 정산 1회성, 재대결 방지·허용, 방 권한과 정원, 백분위 랭크, 관전 입·퇴장과 좌석 전환, 채팅 전달·도배 제한, 규칙 범위 검증, KO 귀속, **릴레이 티켓 상호검증과 서명 정산** |
| `relay` | 13 | 티켓 인증, 신원 위조 차단, WS 두 개로 랭크 경기 완주, 정산 왕복, 매칭·관전·채팅, 재연결과 좌석 회수 |
...
| `browser` | 41 | 실제 Chromium에서 부팅 → 40 LINES 완주 → CPU 대전 → 랭킹·설정·키 리바인드 → 방 생성, **규칙 편집기·채팅·타겟팅 4종·이펙트 발생과 소멸·화면 전환 휘도**, 무오류 확인 |
| `online` | 26 | 탭 **세 개**가 공유 백엔드로 실제 대전: 시드 일치, 가비지 전달, 라운드 판정, 경기 전체 통계, TR 정산, 관전자가 두 보드를 보고 채팅에 참여 |
| `online-relay` | 11 | **하이브리드 전체**: 부트스트랩으로 릴레이 접속, 뜨거운/차가운 경로 분리 확인, 릴레이 경유 랭크 경기, 상대 보드 갱신 시간 측정, 시트에 남은 결과 확인, 연결 끊김과 재연결 |
