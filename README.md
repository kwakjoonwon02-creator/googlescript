...
| | |
|---|---|
| **계정** | 닉네임 + 비밀번호로 가입·로그인. 게스트로 먼저 둘러보다가 그대로 계정으로 전환할 수 있습니다 |
| **경쟁 (Ranked)** | TR 기반 매칭 → FT3 → Glicko-2로 레이팅 정산. 10경기 배치 후 D ~ X+ 랭크 부여. **경기 도중 나가면 패배로 기록되고 TR이 깎입니다** |
...
| **설정** | DAS / ARR / SDF 핸들링, 키 재설정, 고스트·그리드·화면흔들림·이펙트·효과음. **계정에 저장되어 어느 기기에서 로그인하든 따라옵니다** |
| **태블릿** | 소프트 키보드가 올라와도 화면이 잘리지 않고, 창이 짧으면 보드가 잘리는 대신 축소됩니다 |
...
대시보드가 아니라 아케이드 캐비닛을 목표로 했고, 규칙은 네 가지입니다.

**① 직각.** 예전에는 배지·토스트·팝업·패널 눈금·메뉴 행·스코어 스트립까지
전부 -6° 기울어져 있었습니다. 모든 요소에 같은 제스처를 걸면 그건 시그니처가
아니라 벽지가 되고, 인터페이스 전체가 기울어져 있는 것만큼 "찍어낸 티"가 나는
것도 없습니다. 남은 대각선은 화면 전환의 선단 하나뿐입니다.

**② 강조색은 하나.** 화면마다 다른 색을 쓰고 메뉴 행마다 또 다른 색을 쓰던
것을 없앴습니다. 이제 색은 각각 의미가 있습니다 — 주황은 "누르라는 것",
블록 7색은 보드 위, 랭크 색은 데이터, 초록과 빨강은 승패. 그 외에는 아무것도
색을 갖지 않습니다.

**③ 위계.** 똑같이 생긴 다섯 줄은 아무것도 우선하지 않는다는 뜻입니다.
경쟁이 이 앱의 목적이므로 경쟁만 카드로 크게 두고, 나머지 넷은 목록입니다.
화면에서 가장 큰 글자는 장식이 아니라 읽을 가치가 있는 것 — TR 수치입니다.

**④ 자간은 라틴 전용.** 넓은 letter-spacing은 작은 대문자 라벨을 멋있게
만들지만, 한글에 걸면 음절마다 틈을 벌려 `계 정 으 로` 로 만듭니다. 한글이
들어갈 수 있는 곳은 자간을 거의 0으로 두고, 줄바꿈도 어절 단위로 합니다.
브라우저 테스트가 모든 화면을 돌며 이 규칙을 강제합니다.

움직임도 장식이 아니라 상태 변화의 설명입니다.

- 화면 전환마다 기울어진 패널이 화면을 훑고 지나갑니다. 패널은 배경과 거의
  같은 검정이고 강조색은 선단의 5px 선으로만 남습니다 — 채도 높은 색이
  화면 전체를 덮으면 분당 몇 번씩 큰 휘도 변화가 생겨서 눈이 아픕니다.
  그래도 거슬리면 설정에서 끌 수 있습니다
- 매칭 대기 중에는 일곱 칸이 차례로 켜집니다. 피스가 나오는 7-bag입니다
- 버튼 호버는 색이 바뀌는 게 아니라 왼쪽에서 채워집니다. 다만 보조 버튼은
  강조색이 아니라 표면색으로 채워집니다 — 모든 버튼이 강조색으로 번쩍이면
  "취소"가 눌러야 할 것처럼 보입니다
- TR은 이전 값에서 새 값으로 카운트업합니다 (블리츠 점수도)
- 하드 드롭은 떨어진 거리만큼 잔상을 남기고 착지 순간 흰색으로 번쩍입니다
- 헤더는 절대 움직이지 않습니다. 화면이 바뀔 때마다 헤더가 흔들리면
  버튼이 커서 밑에서 도망갑니다

`prefers-reduced-motion: reduce` 를 켠 사용자에게는 장식 애니메이션과
카운트업이 모두 꺼지고 값이 즉시 표시됩니다. 설정에서 **화면 흔들림**,
**이펙트**(하드 드롭 잔상·착지 플래시·라인 클리어 섬광), **화면 전환 효과**
를 각각 따로 끌 수도 있습니다.
---
...
**⑦ 저장소는 두 층**
- `CacheService` — 플레이어 상태, 매칭 대기열, 그리고 아래의 읽기 캐시
  (빠르지만 "최선을 다할 뿐"입니다. 구글이 언제든 지울 수 있습니다)
- 스프레드시트 — 계정, 레이팅, 전적, **그리고 방** (느리지만 영구적이고
  사람이 읽을 수 있음)

캐시에만 있는 데이터는 없습니다. 캐시가 비면 다음 읽기에서 시트로부터
복구되고, 방 안의 사람들은 아무것도 눈치채지 못합니다.
...
- **비밀번호 해시가 약합니다.** Apps Script에는 bcrypt·scrypt·PBKDF2가
  없어 HMAC-SHA256을 계정별 솔트로 1,200회 반복하는 방식입니다. 시트가
  통째로 유출되면 추측 한 번당 1,200번의 해시를 강요할 뿐, 메모리 하드
  함수가 주는 보호는 없습니다. **다른 곳에서 쓰는 비밀번호를 그대로 쓰지
  마세요.**
- **대기열**은 아직 `CacheService`에만 있습니다. 비워지면 대기 중이던
  사람은 다음 폴링에서 자동으로 다시 줄을 서므로 오류로 보이지는 않지만,
  기다린 시간은 초기화됩니다.
...
src/
  ...
  Players.gs          프로필, 솔로 기록
  Accounts.gs         가입·로그인·로그아웃, 비밀번호 해시, 게스트
  Settings.gs         계정별 설정 저장과 범위 검증
  Rooms.gs            방 생애주기, 라운드 상태머신, sync(), 방 영속화
...
| 색상·타입 스케일·모션 토큰 | `css.html` → `:root` |
| 블록 색상·질감 | `js_render.html` → `COLORS`, `block()` |
...
| `server` | 91 | 가입·로그인·로그아웃, 비밀번호 저장과 시도 제한, 게스트와 계정 전환, 구버전 계정 호환, 계정별 설정 저장·범위 검증·키바인드 검증·손상된 값 폴백, 토큰, 한 사람이 랭크 방 두 개에 동시에 잡히지 않는지(느린 폴링·재입장·취소·경기 종료 후), **취소가 이미 날아간 폴링보다 우선하는지, 랭크 기권 시 양쪽 레이팅과 전적·기록, 라운드가 탑아웃으로 끝났는지 연결 끊김으로 끝났는지**, 매칭 페어링과 밴드 확장, 방 상태머신, 연결 끊김, 공격 릴레이와 로그 상한, FT3 정산 1회성, 재대결 방지·허용, 방 권한과 정원, 백분위 랭크, 관전 입·퇴장과 좌석 전환, 채팅 전달·도배 제한·이스케이프, 규칙 범위 검증, KO 귀속, **캐시가 통째로 날아간 뒤에도 이어지는 경기·방 목록·유휴 방 유지** |
...
| `browser` | 59 | 실제 Chromium에서 **가입·로그인 실패·로그아웃·게스트→계정 전환** → 40 LINES 완주 → CPU 대전 → 랭킹·설정·키 리바인드 → 방 생성, 규칙 편집기·채팅·타겟팅 4종·이펙트 발생과 소멸·화면 전환 휘도, 한글에 자간이 벌어지지 않는지·[hidden]이 실제로 숨는지·장식 레이어가 클릭을 삼키지 않는지, 소프트 키보드가 올라왔을 때의 가시 영역·입력창 글자 크기·짧은 창에서의 보드 축소, 설정을 바꾸고 로그아웃한 뒤 다른 기기처럼 다시 들어와도 그대로인지, 보드 테두리가 실제로 보이는 대비인지, **응답이 영영 오지 않는 sync에 갇히지 않는지**, 무오류 확인 |
| `online` | 30 | 탭 **세 개**가 공유 백엔드로 실제 대전: 시드 일치, 가비지 전달, 라운드 판정, 경기 전체 통계, TR 정산, **랭크 기권(경고 문구·취소 시 무손실·양쪽 결과와 TR)·큐 취소가 실제로 취소되는지**, 관전자가 두 보드를 보고 채팅에 참여 |
