...
| `browser` | 50 | 실제 Chromium에서 **가입·로그인 실패·로그아웃·게스트→계정 전환** → 40 LINES 완주 → CPU 대전 → 랭킹·설정·키 리바인드 → 방 생성, 규칙 편집기·채팅·타겟팅 4종·이펙트 발생과 소멸·화면 전환 휘도, **한글에 자간이 벌어지지 않는지·[hidden]이 실제로 숨는지·장식 레이어가 클릭을 삼키지 않는지**, 무오류 확인 |
