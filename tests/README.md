...
| `relay` | 릴레이 티켓 인증과 신원 위조 차단, WS 두 개로 랭크 경기 완주, 서명 정산 왕복, 매칭·관전·채팅, 재연결과 좌석 회수 | 불필요 |
| `online` | 탭 세 개가 공유 백엔드로 실제 대전: 시드 일치, 가비지 전달, 라운드 판정, TR 정산, 관전자가 두 보드를 보고 채팅에 참여 | 필요 |
| `online-relay` | 하이브리드 전체: 부트스트랩 릴레이 접속, 뜨거운/차가운 경로 분리, 릴레이 경유 랭크 경기, 상대 보드 갱신 시간, 시트 기록, 재연결 | 필요 |
...
`online-relay` 는 여기에 실제 릴레이 프로세스를 하나 더 띄웁니다. 웹앱을
먼저 올려 포트를 얻고, 그 주소를 릴레이의 정산 대상으로 넣은 뒤, 릴레이
포트를 다시 목 스크립트 속성에 써 넣어 양쪽을 이어 붙입니다.
